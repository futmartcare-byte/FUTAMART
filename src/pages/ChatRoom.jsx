...
export default function ChatRoom() {
  ...
  const sendMessage = useMutation({
    mutationFn: async (msgData) => {
      ...
      const receiverId = chat?.seller_id === user.id ? chat?.buyer_id : chat?.seller_id; console.log("DEBUG notif:", { receiverId, chatId: chat?.id, sellerId: chat?.seller_id, buyerId: chat?.buyer_id }); await sendChatNotification(receiverId, chat?.seller_id === user.id ? chat.seller_name : chat.buyer_name, msgData.payload_text || "Sent you a message", supabase);
      return newMsg;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["messages", id] });
  ...
  const handleSend = () => {
    ...
    if (replyTo) {
      ...
      msgData.reply_to_text = replyTo.payload_text || "📎 Attachment";
    ...
    if (msg.attachment_type === "voice_note" && msg.attachment_url) {
      ...
      return (
        <button onClick={() => playMsgVoice(msg.id, msg.attachment_url)} className="flex items-center gap-2 py-1">
          ...
          <span className="text-sm">🎙️ Voice note</span>
        </button>
      );
    }
  ...
  return (
    <div className="flex flex-col h-screen max-w-lg mx-auto">
      <div className="glass sticky top-0 z-40 px-3 py-2.5 border-b border-white/5 flex items-center gap-3 shrink-0">
        <button onClick={() => navigate("/chats")} className="p-2 -ml-1 shrink-0">
      ...
      {replyTo && (
        <div className="glass border-t border-white/5 px-3 py-2 flex items-center gap-2 shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground truncate">{replyTo.payload_text || "📎 Attachment"}</p>
          </div>
