        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            {form.images.length < 5 && (
              <FileUploadWithCompress multiple={true} onFile={async (file) => { setUploading(true); try { const url = await uploadToCloudinary(file, "futmart/listings"); setForm(prev => { if (prev.images.length >= 5) { toast.error("Maximum 5 images"); return prev; } return { ...prev, images: [...prev.images, url] }; }); } catch { toast.error("Upload failed"); } finally { setUploading(false); } }} accept="image/*"><div className="aspect-square rounded-xl glass flex flex-col items-center justify-center cursor-pointer hover:brightness-110">{uploading ? <Loader2 className="w-5 h-5 text-muted-foreground animate-spin" /> : <><Camera className="w-5 h-5 text-muted-foreground" /><span className="text-[10px] text-muted-foreground mt-1">Add</span></>}</div></FileUploadWithCompress>
            )}
          </div>
          {uploading && (
